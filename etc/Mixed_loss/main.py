...
class SemiLoss(object):
    def __call__(self, mixup_idx, outputs_x, targets_x, outputs_u, targets_u, epoch, final_epoch):
        if len(mixup_idx)!=0:
            probs_u = torch.softmax(outputs_u, dim=1)
            mixup_idx = np.asarray(mixup_idx)
            #print(mixup_idx)
            mixup_idx = np.concatenate((mixup_idx , mixup_idx+opts.batchsize2))
            #print(mixup_idx)

            Lx = -torch.mean(torch.sum(F.log_softmax(outputs_x, dim=1) * targets_x, dim=1))
            Lt = torch.mean((probs_u[mixup_idx] - targets_u[mixup_idx])**2)
            Lu = torch.mean((probs_u - targets_u)**2)
            return Lx, Lt*opts.lambda_u, Lu, opts.lambda_u * linear_rampup(epoch, final_epoch)
        else:
            probs_u = torch.softmax(outputs_u, dim=1)
            Lx = -torch.mean(torch.sum(F.log_softmax(outputs_x, dim=1) * targets_x, dim=1))
            Lu = torch.mean((probs_u - targets_u)**2)
            return Lx, -1, Lu, opts.lambda_u * linear_rampup(epoch, final_epoch)
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        loss_x, loss_t, loss_un, weigts_mixing = criterion(mixup_idx, logits_x, mixed_target[:batch_size], logits_u, mixed_target[batch_size:], epoch+batch_idx/len(train_loader), 20)
        if loss_t != -1:
            loss = loss_x + weigts_mixing * loss_un + loss_t
        else:
        ...
        if loss_t !=-1:
            losses_t.update(loss_t.item(), inputs_x.size(0))
        ...
        nsml.report(summary=True, losses_x = losses_x.avg, losses_un = losses_un.avg*weigts_mixing, losses_t = losses_t.avg, step = epoch+batch_idx/len(train_loader))
