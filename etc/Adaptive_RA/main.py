...
parser.add_argument('--lambda-u', default=1500, type=float)
...
def main():
    ...
    if opts.mode == 'train':
        ...
        train_loader = torch.utils.data.DataLoader(
            SimpleImageLoader(DATASET_PATH, 'train', train_ids, RandAugment(2,5),
                              transform=transforms.Compose([
                                  transforms.Resize(opts.imResize),
        ...
        unlabel_loader = torch.utils.data.DataLoader(
            SimpleImageLoader(DATASET_PATH, 'unlabel', unl_ids, RandAugment(2,5),
                              transform=transforms.Compose([
                                  transforms.Resize(opts.imResize),
                                  transforms.RandomResizedCrop(opts.imsize),
                                  transforms.RandomHorizontalFlip(),
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu, train_acc_top1_val, train_loss_val, train_loss_x_val, train_loss_un_val):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        try:
            ...
            inputs_u1, inputs_u2, inputs_w1, inputs_w2 = data
        except:
            unlabeled_train_iter = iter(unlabel_loader)
            data = unlabeled_train_iter.next()
            inputs_u1, inputs_u2, inputs_w1, inputs_w2 = data
        ...
        if use_gpu :
            ...
            inputs_u1, inputs_u2, inputs_w1, inputs_w2 = inputs_u1.cuda(), inputs_u2.cuda(), inputs_w1.cuda(), inputs_w2.cuda()
        inputs_x, targets_x = Variable(inputs_x), Variable(targets_x)
        inputs_u1, inputs_u2, inputs_w1, inputs_w2 = Variable(inputs_u1), Variable(inputs_u2), Variable(inputs_w1), Variable(inputs_w2)
        ...
        with torch.no_grad():
            embed_u1, pred_u1 = model(inputs_w1) #여기서 두개
            embed_u2, pred_u2 = model(inputs_w2) #mixmatch
