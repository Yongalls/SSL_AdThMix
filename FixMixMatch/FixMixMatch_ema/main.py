...
def main():
    ...
    if opts.mode == 'train':
        ...
        print("Title: {}".format("Fix - MixMatch + Ema (Yongalls)"))
        ...
        print("Optimizer: {}, Scheduler: {}".format("SGD with momentum 0.9, wd 0.0004", "none"))
        ...
        print("Etc: {}".format("No interleaving. IDK // threshold scheduling linearly, min:(0.4).  Without learning rate cheduling, EMA model added"))
        ...
        for epoch in range(opts.start_epoch, opts.epochs + 1):
            ...
            loss, _, _ = train(opts, train_loader, unlabel_loader, model, ema_model, train_criterion, optimizer, epoch, use_gpu)
            #scheduler.step()

            print('start validation')
            acc_top1, acc_top5 = validation(opts, validation_loader, model, epoch, use_gpu)
            ema_acc_top1, ema_acc_top5 = validation(opts, validation_loader, ema_model, epoch, use_gpu)
            is_best = ema_acc_top1 > best_acc
            best_acc = max(ema_acc_top1, best_acc)
            nsml.report(summary=True, train_loss= loss, val_acc_top1= acc_top1, val_acc_top5=acc_top5, ema_val_acc_top1= ema_acc_top1, ema_val_acc_top5=ema_acc_top5, step=epoch)
            if is_best:
...
def train(opts, train_loader, unlabel_loader, model, ema_model, criterion, optimizer, epoch, use_gpu):
    losses = AverageMeter()
    ...
    for batch_idx in range(len(train_loader)):
        ...
        with torch.no_grad():
            embed_u1, pred_u1 = ema_model(inputs_u1)
            embed_u2, pred_u2 = ema_model(inputs_u2)
        ...
        with torch.no_grad():
            ...
            _, ema_pred_x1 = ema_model(inputs_x)
        ...
        ema_acc_top1b = top_n_accuracy_score(targets_org.data.cpu().numpy(), ema_pred_x1.data.cpu().numpy(), n=1)*100
        ...
        if batch_idx % opts.log_interval == 0:
            print('Train Epoch:{} [{}/{}] Loss:{:.4f}({:.4f}) Top-1:{:.2f}%({:.2f}%) Top-5:{:.2f}%({:.2f}%) EMA_Top-1:{:.2f}%({:.2f}%) EMA_Top-5:{:.2f}%({:.2f}%)'.format(
                epoch, batch_idx *inputs_x.size(0), len(train_loader.dataset), losses.val, losses.avg, acc_top1.val, acc_top1.avg, acc_top5.val, acc_top5.avg, ema_acc_top1.val, ema_acc_top1.avg, ema_acc_top5.val, ema_acc_top5.avg))
    ...
    nsml.report(summary=True, train_acc_top1= avg_top1, train_acc_top5=avg_top5, ema_train_acc_top1= ema_avg_top1, ema_train_acc_top5=ema_avg_top5, step=epoch, good_unlabeled = good_ulb.avg)
