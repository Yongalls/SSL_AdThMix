...
def criterion_pi(pred1, pred2):
    probs_1 = torch.softmax(pred1, dim=1)
    probs_2 = torch.softmax(pred2, dim=1)
    loss = torch.mean((probs_1 - probs_2)**2)
    return loss
...
parser.add_argument('--lambda-pi', default=10, type=float)
...
def main():
    ...
    if opts.mode == 'train':
        #nsml.load(checkpoint ='Res18baseMM_e49', session='kaist_15/fashion_eval/131')
        model.train()
        # Set dataloader
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,  milestones=[80], gamma=0.09)
        ...
        print("Title: {}".format("FixMixmatch + Pi (Yongalls)"))
        ...
        print("Optimizer: {}, Scheduler: {}".format("SGD with momentum 0.9, wd 0.0004", "MultiStepLR with [80], 0.09"))
        print("Other necessary Hyperparameters: {}, , lambda-pi: {}".format("Batchsize for unlabeled is 75., lambda-u not changed in overall training step.", opts.lambda_pi))
        print("Details: {}".format("Experiment for thresholding some unlabeled examples in pseudo labeling. current threshold 0.9"))
        print("Etc: {}".format("No interleaving. IDK // same threshold scheduling. Added lr schedueling. Added entropy minimization. Do rampup for consistency loss"))
        ...
        for epoch in range(opts.start_epoch, opts.epochs + 1):
            ...
            nsml.report(summary=True, val_acc_top1= acc_top1, val_acc_top5=acc_top5, step=epoch)
            if is_best:
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu):
    ...
    losses_pi_weighted = AverageMeter()
    ...
    for batch_idx in range(len(train_loader)):
        ...
        if epoch < 40:
            ...
            threshold = max(0.9 - (epoch - 40)/300, 0.1)
        ...
        loss_pi = Variable(torch.tensor(0))
        if len(pi_idx) > 1 and epoch >= 10:
            inputs_u1_pi = inputs_u1[pi_idx]
            inputs_u2_pi = inputs_u2[pi_idx]
        
            _, outputs_u1 = model(inputs_u1_pi)
            _, outputs_u2 = model(inputs_u2_pi)
            loss_pi = criterion_pi(outputs_u1, outputs_u2)

        # mixup
        inputs_u1_mixup = inputs_u1[mixup_idx]
        inputs_u2_mixup = inputs_u2[mixup_idx]
        targets_u_mixup = targets_u[mixup_idx]

        good_ulb.update(len(mixup_idx))

        all_inputs = torch.cat([inputs_x, inputs_u1_mixup, inputs_u2_mixup], dim=0)
        all_targets = torch.cat([targets_x, targets_u_mixup, targets_u_mixup], dim=0)
        ...
        if len(pi_idx)!=batch_size_u:
            ...
            loss_x, loss_un, weights_mixing = criterion(logits_x, mixed_target[:batch_size], logits_u, mixed_target[batch_size:], epoch+batch_idx/len(train_loader), opts.epochs)
            loss_pi_weighted = weights_mixing * opts.lambda_pi * loss_pi
            loss = loss_x + opts.lambda_u * loss_un + loss_pi_weighted
            losses.update(loss.item(), inputs_x.size(0))
            losses_x.update(loss_x.item(), inputs_x.size(0))
            losses_un.update(loss_un.item(), inputs_x.size(0))
            weight_scale.update(weights_mixing, inputs_x.size(0))
            ...
            loss_pi_weighted = opts.lambda_u * linear_rampup(epoch+batch_idx/len(train_loader), opts.epochs) * opts.lambda_pi * loss_pi
            loss = loss_x + loss_pi_weighted
            losses.update(loss.item(), inputs_x.size(0))
            losses_x.update(loss_x.item(), inputs_x.size(0))
        ...
        losses_pi.update(loss_pi.item(), inputs_x.size(0))
        losses_pi_weighted.update(loss_pi_weighted.item(), inputs_x.size(0))
        ...
        if batch_idx % opts.log_interval == 0:
            print('Train Epoch:{} [{}/{}] Loss:{:.4f}({:.4f}) Loss_pi: {:.4f}({:.4f}) Top-1:{:.2f}%({:.2f}%) Top-5:{:.2f}%({:.2f}%) '.format(
                epoch, batch_idx *inputs_x.size(0), len(train_loader.dataset), losses.val, losses.avg, losses_pi.val, losses_pi.avg, acc_top1.val, acc_top1.avg, acc_top5.val, acc_top5.avg))
    ...
    nsml.report(summary=True, train_acc_top1= avg_top1, train_acc_top5=avg_top5, step=epoch, good_unlabeled = good_ulb.avg, consistency_loss = losses_pi.avg, consistency_loss_weighted = losses_pi_weighted.avg, train_loss = avg_loss, train_loss_without_consistencyloss = avg_loss - losses_pi_weighted.avg)
