...
def interleave(xy, batch, batch_u, back):
    assert len(xy) == 3
    nu = len(xy) - 1
    offsets_x = interleave_offsets(batch, nu)
    #print(offsets_x)
    offsets_u1 = interleave_offsets(batch_u, nu)
    #print(offsets_u1)
    offsets_u2 = interleave_offsets(batch_u, nu)
    #print(offsets_u2)
    offsets = [offsets_x, offsets_u1, offsets_u2]

    if back == True:
        back_x = [0, offsets_x[1], offsets_x[1] + offsets_u1[2]-offsets_u1[1], offsets_x[1]+offsets_u1[2]-offsets_u1[1] + offsets_u2[3]-offsets_u2[2]]
        back_u1 = [0, offsets_u1[1], offsets_u1[1] + offsets_x[2]-offsets_x[1], offsets_u1[1] + offsets_x[2]-offsets_x[1] + offsets_u1[3]-offsets_u1[2]]
        back_u2 = [0, offsets_u2[1], offsets_u2[2], offsets_u2[2] + offsets_x[3]-offsets_x[2]]
        offsets = [back_x, back_u1, back_u2]

    xy = [[xy[v][offsets[v][p]:offsets[v][p + 1]] for p in range(nu + 1)] for v in range(len(xy))]
    
    #print("before", len(xy))
    #for i in range(len(xy)):
    #    for j in range(len(xy[i])):
    #        print(i,j,xy[i][j].size())
...
def main():
    ...
    if opts.mode == 'train':
        ...
        print("Title: {}".format("FixMixMatch (Interleaving)"))
        ...
        print("Etc: {}".format("Include interleaving. IDK // threshold scheduling same as 79.  Without learning rate cheduling"))
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        mixed_input = [mixed_input[:batch_size], mixed_input[batch_size:batch_size + len(mixup_idx)], mixed_input[batch_size + len(mixup_idx):]]
        #print("list: ",len(mixed_input), "mixup_idx", len(mixup_idx))
        #for i_ in range(len(mixed_input)):
        #    print(i_, mixed_input[i_].size())
        
        if len(mixup_idx) != 0:
            mixed_input = interleave(mixed_input, batch_size, len(mixup_idx), False)
        ...
        if len(mixup_idx) != 0:
            ...
            logits = interleave(logits, batch_size, len(mixup_idx), True)
            logits_x = logits[0]
